...
class BatchActionsButtons extends Component {
    ...
    render() {
        const actionsDisabled = this.props.points.length === 0;
        return (
            <DropdownButton bsStyle="primary" title="Batch actions" id="actions-dropdown" disabled={actionsDisabled}>
                <MenuItem onClick={() => this.handleBatchSubscribe()}>Subscribe</MenuItem>
                <MenuItem onClick={() => this.handleBatchUnsubscribe()}>Unsubscribe</MenuItem>
                <MenuItem onClick={() => this.props.dispatch(showModal("Interpolate", Interpolate, {points: this.props.points}))}>Interpolate</MenuItem>
                <MenuItem onClick={() => this.props.dispatch(showModal("Backfill", Backfill, {points: this.props.points}))}>Backfill</MenuItem>
            </DropdownButton>
        )
    }
}
