...
from model.kepware.models import Feed
...
def test_feeds_search_kepware(api_test_kepware_client, postgres_session_kepware, search_request, expected_response):
    postgres_session_kepware.add_all(copy_db_objects([feed_1, feed_2, feed_3, feed_4]))
    response = parse_response(api_test_kepware_client.post('/api/v1/feeds/search', data=json.dumps(search_request),
